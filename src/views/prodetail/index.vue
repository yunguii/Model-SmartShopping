<template>
  <div class="prodetail">
    ...
    <div class="comment">
      ...
      <div class="comment-list">
        <div class="comment-item" v-for="item in comment.list" :key="item.comment_id">
          <div class="top">
            <img :src="item.user.avatar_url ||defAvatar" alt="">
    ...
    <!-- 商品描述 -->
    <div class="desc" v-html="list.content">
    </div>
    ...
    <van-action-sheet v-model="showPannel" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
        ...
        <div class="num-box">
          <span>数量</span>
          <GoodsNum v-model="number"></GoodsNum>
       </div>
       <div class="showbtn" v-if="list.stock_total > 0">
         <div class="btn" v-if="mode === 'cart'" @click="addCar">加入购物车</div>
         <div class="btn now"  v-if="mode === 'buyNow'" @click="buyGoods">立刻购买</div>
       </div>
...
export default {
  ...
  methods: {
    ...
    async addCar () {
      ...
      const { data: { data } } = await addCart(this.list.goods_id, this.number, this.list.skuList[0].goods_sku_id)
  ...
  async created () {
    const { data: { data } } = await getProDetail(this.getId)
    this.list = data.detail
    this.images = data.detail.goods_images
    const res = await getProComment(this.getId)
